...
const _sfc_main = {
  ...
  onLoad() {
    ...
    common_vendor.index.checkIsSupportSoterAuthentication({
      ...
      fail: (err) => {
        common_vendor.index.__f__("log", "at pages/ucenter/settings/settings.vue:73", err);
      }
    });
  },
  onShow() {
  ...
  methods: {
    ...
    startSoterAuthentication(checkAuthMode) {
      common_vendor.index.__f__("log", "at pages/ucenter/settings/settings.vue:100", checkAuthMode);
      let title = {
      ...
      this.checkIsSoterEnrolledInDevice({
        ...
        common_vendor.index.__f__("log", "at pages/ucenter/settings/settings.vue:111", checkAuthMode, title);
        common_vendor.index.startSoterAuthentication({
          ...
          complete: (res) => {
            common_vendor.index.__f__("log", "at pages/ucenter/settings/settings.vue:118", res);
          },
          success: (res) => {
            common_vendor.index.__f__("log", "at pages/ucenter/settings/settings.vue:121", res);
            if (res.errCode == 0) {
          ...
          fail: (err) => {
            common_vendor.index.__f__("log", "at pages/ucenter/settings/settings.vue:141", err);
            common_vendor.index.__f__("log", "at pages/ucenter/settings/settings.vue:142", `认证失败:${err.errCode}`);
            common_vendor.index.showToast({
    ...
    checkIsSoterEnrolledInDevice({
      ...
      return new Promise((resolve, reject) => {
        common_vendor.index.checkIsSoterEnrolledInDevice({
          checkAuthMode,
          success: (res) => {
            common_vendor.index.__f__("log", "at pages/ucenter/settings/settings.vue:160", res);
            if (res.isEnrolled) {
              return resolve(res);
            }
            common_vendor.index.showToast({
          ...
          fail: (err) => {
            common_vendor.index.__f__("log", "at pages/ucenter/settings/settings.vue:171", err);
            common_vendor.index.showToast({
    ...
    clearTmp() {
      ...
      common_vendor.index.getSavedFileList({
        success: (res) => {
          if (res.fileList.length > 0) {
            common_vendor.index.removeSavedFile({
              ...
              complete: (res2) => {
                common_vendor.index.__f__("log", "at pages/ucenter/settings/settings.vue:203", res2);
        ...
        complete: (e) => {
          common_vendor.index.__f__("log", "at pages/ucenter/settings/settings.vue:220", e);
        }
      });
    },
    changeLanguage() {
      common_vendor.index.__f__("log", "at pages/ucenter/settings/settings.vue:225", "语言切换");
      common_vendor.index.showActionSheet({
        itemList: ["English", "简体中文"],
        success: (res) => {
          common_vendor.index.__f__("log", "at pages/ucenter/settings/settings.vue:229", res.tapIndex);
