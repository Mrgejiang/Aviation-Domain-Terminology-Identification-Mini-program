  ...
  async updateUserInfo(data2 = false) {
    if (data2) {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at uni_modules/uni-id-pages/common/store.js:59", e.message, e.errCode);
      }
    }
  },
  async setUserInfo(data2, {
    cover
  } = {
    cover: false
  }) {
  ...
  async logout() {
    if (common_vendor.er.getCurrentUserInfo().tokenExpired > Date.now()) {
      try {
        ...
        common_vendor.index.__f__("error", "at uni_modules/uni-id-pages/common/store.js:82", e);
      }
    }
      ...
      return common_vendor.index.redirectTo({
        ...
        fail: (err1) => {
          common_vendor.index.switchTab({
            url: uniIdRedirectUrl,
            fail: (err2) => {
              common_vendor.index.__f__("log", "at uni_modules/uni-id-pages/common/store.js:116", err1, err2);
            }
          });
        }
      });
  ...
  loginSuccess(e = {}) {
    ...
    if (uni_modules_uniIdPages_config.config.setPasswordAfterLogin && !passwordConfirmed) {
      return common_vendor.index.redirectTo({
        ...
        fail: (err) => {
          common_vendor.index.__f__("log", "at uni_modules/uni-id-pages/common/store.js:162", err);
        }
      });
    }
    if (autoBack) {
